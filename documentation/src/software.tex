...
\end{itemize}
A schematic of the whole process can be seen here:
% camera - undistortion - ball detection - prediction where the ball will be (y coordinate of the ball when it reaches the player) - motor control
%                                        |-predict when the ball will reach the player
...
Using the undistort function provided by opencv is not fast enough for our needs, because the program only has 3ms to do the whole image processing and motor controlling.
...
\end{equation}
Now I only have to convert the $\vec{r}_\text{intersection}$ to real world coordinates and I get the position where the ball will end up, which the program sends to the arduino to move the stepper motor to this position.
